...
/// Configuration for validator uploads to an indexer.
#[derive(Deserialize, Serialize)]
...
mod tests {
    ...
    #[test_traced]
    fn test_indexer_without_durable_block_uploads_skips_drainer() {
        let n = 5;
        let required_container = 10;
        let executor = Runner::timed(Duration::from_secs(30));
        executor.start(|mut context| async move {
            ...
            // Without durable raw block uploads enabled, failed cert uploads do
            // not trigger the drainer.
            assert!(!indexer
                .block_upload_seen
                .load(std::sync::atomic::Ordering::Relaxed));
            assert_eq!(
                indexer
                    ...
                    .load(std::sync::atomic::Ordering::SeqCst),
                0,
                "raw block uploads should stay disabled without durable upload support",
            );
        });
    }
