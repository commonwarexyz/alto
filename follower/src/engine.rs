...
pub(crate) const IMMUTABLE_ITEMS_PER_SECTION: NonZero<u64> = NZU64!(262_144);
pub(crate) const FREEZER_TABLE_RESIZE_FREQUENCY: u8 = 4;
pub(crate) const FREEZER_TABLE_RESIZE_CHUNK_SIZE: u32 = 2u32.pow(16); // 3MB
pub(crate) const FREEZER_JOURNAL_TARGET_SIZE: u64 = 1024 * 1024 * 1024; // 1GB
pub(crate) const FREEZER_JOURNAL_COMPRESSION: Option<u8> = Some(3);
...
pub(crate) const BUFFER_POOL_PAGE_SIZE: NonZero<u16> = NZU16!(4_096); // 4KB
pub(crate) const BUFFER_POOL_CAPACITY: NonZero<usize> = NZUsize!(8_192); // 32MB
pub const DEFAULT_MAX_REPAIR: NonZero<usize> = NZUsize!(256);
const VIEW_RETENTION_TIMEOUT: ViewDelta = ViewDelta::new(2560);
const DEQUE_SIZE: usize = 10;
pub(crate) const BLOCKS_FREEZER_TABLE_INITIAL_SIZE: u32 = 2u32.pow(21); // 100MB
pub(crate) const FINALIZED_FREEZER_TABLE_INITIAL_SIZE: u32 = 2u32.pow(21); // 100MB

pub type FinalizedArchive<E> = immutable::Archive<E, Digest, Finalization>;
...
/// Create the archives and buffer pool used by the engine.
///
/// This is separated from [Engine::new] so the archives can be pre-filled
/// by the [backfiller](crate::backfill::Backfiller) before starting marshal.
pub async fn create_archives<E>(
    context: &E,
    scheme: &Scheme,
) -> (FinalizedArchive<E>, BlockArchive<E>, CacheRef)
where
    E: commonware_runtime::Clock + GClock + Rng + CryptoRng + Spawner + Storage + Metrics,
{
    ...
    let finalizations_by_height = immutable::Archive::init(
        ...
        immutable::Config {
            ...
            freezer_table_partition: "follower-finalizations-by-height-freezer-table".to_string(),
    ...
    let finalized_blocks = immutable::Archive::init(
        ...
        immutable::Config {
            ...
            freezer_key_page_cache: page_cache.clone(),
            freezer_value_partition: "follower-finalized-blocks-freezer-value-journal".to_string(),
            freezer_value_target_size: FREEZER_JOURNAL_TARGET_SIZE,
    ...
    (finalizations_by_height, finalized_blocks, page_cache)
}
...
where
    E: commonware_runtime::Clock + GClock + Rng + CryptoRng + Spawner + Storage + Metrics,
{
    /// Create a new [Engine] with pre-created archives.
    pub async fn new(
        mut context: E,
        scheme: Scheme,
